...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc4_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{20,20,10},{30,30,30},{40,40,30}};
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results4-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),2,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_4.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,1,"test4",f->frags_());
