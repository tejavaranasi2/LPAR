...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc15_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{10,10,10,10,40},{20,20,13,20,30},{30,30,11,30,30}};
    ...
    coords.push_back(std::pair<double,double>(15,25));
    ...
    for(int i=0;i<2;i++){
        ...
        e_c[0]=(((v[0]*-1+circles[i][0])^2)*(coeff[i][0])+((v[1]*-1+circles[i][1])^2)*(coeff[i][1])+((v[2]*-1+circles[i][2])^4)*(coeff[i][2])+((v[3]*-1+circles[i][3])^2)*(coeff[i][3]))*(-1)+(circles[i][4]*circles[i][4]);
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results15-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),4,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_15.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,2,"test15",f->frags_());
