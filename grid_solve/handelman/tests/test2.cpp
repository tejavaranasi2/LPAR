...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc2_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{1,2,4},{-2,0,4}};
    std::vector<std::vector<double> > coeff={{1,1},{1,1}};
    std::vector<solver*> sls_pos,sls_neg;
    std::string t;
    t=v.add_var(4,true,"t4");
    ll* ans=NULL;
    ...
    id_map[std::pair<int,int>(coords.size(),1)]=v(t,2).prim_key->get_id();
    coords.push_back(std::pair<double,double>(-4,4));
    ...
    for(int i=0;i<2;i++){
        ...
        e1_.add_lhs(v(t,2)+v[0]*-1);

        e1_.add_lhs(v[1]+v(t,1)*-1);
        e1_.add_lhs(v(t,3)+v[1]*-1);
        ...
        e2_.add_lhs(v(t,2)+v[0]*-1);

        e2_.add_lhs(v[1]+v(t,1)*-1);
        e2_.add_lhs(v(t,3)+v[1]*-1);
        ...
        e_c[0]=(((v[0]*-1+circles[i][0])^2)*(coeff[i][0])+((v[1]*-1+circles[i][1])^2)*(coeff[i][1]))*(-1)+(circles[i][2]*circles[i][2]);
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results2-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),2,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_2.json",coords);
    HEAD->write_code("/home/poqer/Desktop/tests/tc_2.txt",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,1,"test2",f->frags_());
