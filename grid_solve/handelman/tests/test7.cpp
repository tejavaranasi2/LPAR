...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc7_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{200,300,200,150},{300,400,200,270}};
    std::vector<std::vector<double> > coeff={{1,4,1},{4,1,1}};
    ...
    coords.push_back(std::pair<double,double>(0,400));
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results7-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),3,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_7.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,2,"test7",f->frags_());
