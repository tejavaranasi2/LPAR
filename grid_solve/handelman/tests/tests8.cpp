...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc8_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{20,30,20,15},{30,40,25,27},{40,35,25,30}};
    std::vector<std::vector<double> > coeff={{1,1,1},{1,1,1},{1,1,1}};
    std::vector<solver*> sls_pos,sls_neg;
    std::string t;
    t=v.add_var(6,true,"t4");
    ll* ans=NULL;
    ...
    id_map[std::pair<int,int>(coords.size(),1)]=v(t,3).prim_key->get_id();
    coords.push_back(std::pair<double,double>(0,80));
    ...
    for(int i=0;i<3;i++){
        ...
        e1_.add_lhs(v(t,3)+v[0]*-1);

        e1_.add_lhs(v[1]+v(t,1)*-1);
        e1_.add_lhs(v(t,4)+v[1]*-1);
        
        e1_.add_lhs(v[2]+v(t,2)*-1);
        e1_.add_lhs(v(t,5)+v[2]*-1);
        
        
       
        
        e2_.add_lhs(v[0]+v(t,0)*-1);
        e2_.add_lhs(v(t,3)+v[0]*-1);

        e2_.add_lhs(v[1]+v(t,1)*-1);
        e2_.add_lhs(v(t,4)+v[1]*-1);

        e2_.add_lhs(v[2]+v(t,2)*-1);
        e2_.add_lhs(v(t,5)+v[2]*-1);
        ...
        e_c[0]=(((v[0]*-1+circles[i][0])^2)*(coeff[i][0])+((v[1]*-1+circles[i][1])^2)*(coeff[i][1])+((v[2]*-1+circles[i][2])^2)*(coeff[i][2]))*(-1)+(circles[i][3]*circles[i][3]);
   ...
   //  }
    ...
    f->run_threads(80000,0.7,dump_file,NULL,10,true);
    f->recompute_confidence(e_check);//makes SMT calls::
    if(flag==2){
      f->frags_()=operation(new_l,f->frags_(),0);
    }
    f->frags_()->print("/results_prog_sys/results8-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),3,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_8.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,2,"test8",f->frags_());
