...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc12_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{10,20,40},{-20,23,40}};
    ...
    coords.push_back(std::pair<double,double>(-40,40));
    ...
    for(int i=0;i<2;i++){
        ...
        e_c[0]=(((v[0]*-1+circles[i][0])^2)*(coeff[i][0])+((v[1]*-1+circles[i][1])^4)*(coeff[i][1]))*(-1)+(circles[i][2]*circles[i][2]);
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results12-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),2,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_12.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,1,"test12",f->frags_());
