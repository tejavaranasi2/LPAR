...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc1_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    ...
    for(int i=0;i<3;i++){
        ...
        if(i==0){
           e_c[0]=((v[0]^2)*3+v[0]*v[1]*2+(v[1]^2)+(v[2]^2)+(v[0]*-1)+v[1])*-1+(7);
        }
        else if(i==1){
           e_c[0]=((v[0]^2)+(v[1]^2)+((v[2]+(-1))^2))*-1+(4);
        }
        else{
           e_c[0]=(((v[0]+(-1))^2)+((v[1]+(-1))^2)+((v[2]+(-2))^2)+(v[0]*v[1]*(-1))+(v[0]*v[2]*(-1)))*-1+(4);
        }
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results1-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),3,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_1.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,2,"test1",f->frags_());
