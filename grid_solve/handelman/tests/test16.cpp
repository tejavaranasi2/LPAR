...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc16_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{0,0,std::pow(45,0.5)},{0,0,std::pow(30,0.5)}};
    ...
    coords.push_back(std::pair<double,double>(-5,5));
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results16-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),2,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_16.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,1,"test16",f->frags_());
