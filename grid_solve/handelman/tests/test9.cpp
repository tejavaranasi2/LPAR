...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc9_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{20,30,20,15},{30,40,25,27},{40,35,25,30}};
    std::vector<std::vector<double> > coeff={{1,4,2},{2,1,2},{1,3,1}};
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results9-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),3,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_9.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,2,"test9",f->frags_());
