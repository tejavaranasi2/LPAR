...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc3_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{1,2,8},{-2,0,8}};
    std::vector<std::vector<double> > coeff={{4,1},{1,4}};
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results3-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),2,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_3.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,1,"test3",f->frags_());
