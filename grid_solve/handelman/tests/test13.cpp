...
int main(int argv, char* argc[]){
   ...
   int ctr_i=1;
    LOG_FILE=std::ofstream("../logs/output_tc13_prog_sys_parallel10-"+std::to_string(ctr_i)+".log");
    double ctr=ctr_i/2;
    std::vector<std::vector<double> > circles={{20,30,20,10},{30,40,25,25}};
    std::vector<std::vector<double> > coeff={{1,1,1},{1,1,1}};
    ...
    for(int i=0;i<2;i++){
        ...
        e_c[0]=(((v[0]*-1+circles[i][0])^2)*(coeff[i][0])+((v[1]*-1+circles[i][1])^2)*(coeff[i][1])+((v[2]*-1+circles[i][2])^4)*(coeff[i][2]))*(-1)+(circles[i][3]*circles[i][3]);
        ...
        e1_.set_max_degree(4);
        e1_.solve(3,0);

        e2_.set_max_degree(4);
        e2_.solve(3,0);
   ...
   //  }
    ...
    f->frags_()->print("/results_prog_sys/results13-"+(std::string)argc[4]+".csv");

    trie_node* HEAD= trie_node::make_trie(f->frags_(),3,coords);
    HEAD->write_json("/home/poqer/Desktop/tests/tc_13.json",coords);
    // for(int i=0;i<3;i++){
    ...
    sl_parser::make_test_case(e_check,coords,v,2,"test13",f->frags_());
